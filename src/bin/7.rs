...
fn main() {
    ...
    for line in lines.iter().skip(1) {
        ...
        if let Ok(size) = part1.parse::<u32>() {
            // if the line is a file
            for folder in &current_folders {
                // add its size to the current directory and its parents
            ...
            if part3 == ".." {
                ...
                // part 3 is a folder name
                current_folders.push(format!("{}/{}", current_folders.last().unwrap(), part3));
                // move down a folder
            }
        }
    }

    let a_total = folder_sizes
        .values()
    ...
    let b_folder_size = folder_sizes
        .values()
